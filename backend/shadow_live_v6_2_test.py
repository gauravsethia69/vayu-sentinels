...
MODEL_PATH = MODEL_DIR / "skyguard_pytorch_multiclass_v6_2_recovery_aware.pt"
OUT_CSV = BASE_DIR / "shadow_live_v6_2_output.csv"
...
def shadow_predict(node_id, history, state, model, package):
    ...
    elif prediction in ("spike", "drift"):
        if confidence >= CONFIRM_CONFIDENCE and state["same_count"] >= CONFIRM_WINDOWS:
            confirmed = True
            confirmed_fault = prediction
...
def main():
    ...
    print("=" * 80)
    print("SkyGuard V6.2 Shadow Live Test")
    print("=" * 80)
                ...
                for node in NODES:
                    try:
                        ...
                        print(
                            ...
                            f"shadow=V6.2:{shadow_prediction}:{shadow_conf_txt}:"
