...
if not _env_enabled("SKYGUARD_PYTORCH_ENABLED", "true"):
    MODEL_VERSION = "V6.3"
    ...
    class DisabledPyTorchDetector:
        def __init__(self):
            self.loaded = False
            self.error = "PyTorch disabled by SKYGUARD_PYTORCH_ENABLED=false"
            self.predictions = 0
            self.last_prediction_at = None

        def process(self, node_id: str, sensors: dict):
            return {
                "supported": node_id in SUPPORTED_NODES,
                "model": MODEL_NAME,
                ...
                "probabilities": {},
                "safety_layer": "disabled_on_this_runtime",
                "gate_reason": self.error,
                "readings": 0,
                "error": self.error,
            }
        ...
        def status(self):
            return {
                ...
                "routing": {
                    "AWS_001": "PyTorch V6.3 disabled on this runtime",
                    "AWS_002": "PyTorch V6.3 disabled on this runtime",
                    "AWS_003": "PyTorch V6.3 disabled on this runtime",
                },
                "nodes": {},
                "error": self.error,
    ...
    pytorch_detector = DisabledPyTorchDetector()
